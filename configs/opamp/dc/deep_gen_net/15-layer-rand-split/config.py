...
inds = torch.randperm(len(dset))
sample_data = dset[0]
splits = dset.splits
train_dset = dset[inds[splits['train']]]
valid_dset = dset[inds[splits['valid']]]
test_dset = dset[inds[splits['test']]]
...
exp_name = f'Opamp_DC_DeepGEN_RandSplit_h{hidden_channels}_nl{num_layers}_bs{train_batch_size}_lr{lr:.0e}_{activation}'
