...
sample_data = dset[0]
splits = dset.splits
train_dset = dset[splits['train']].shuffle()
...
exp_name = f'Opamp_DC_InterleavePtTrsfGCNNet_({num_layers}xgcn+{transformer_depth}xtf)x{depth}'
...
import numpy as np
info = torch.load(Path(dset.processed_dir) / 'graphs.pt')

nx_g = to_networkx(sample_data, to_undirected=True)
shortest_path_lengths = nx.all_pairs_shortest_path_length(nx_g)
shortest_path_lengths = sorted(shortest_path_lengths, key=lambda x: x[0])
shortest_path_arr = [list(dict(sorted(item[1].items(), key= lambda kv: kv[0])).values()) for item in shortest_path_lengths]
shortest_path_arr = np.array(shortest_path_arr)
...
mdl_config = ParamDict(
    ...
    output_labels={'vdc': 1},
    proj_n_layers=3,
    node_str=next(iter(info['graph_nodes'].values())),
    dist_matrix=shortest_path_arr,
)
